...
class SPOT:
    ...
    def initialize(self, level=0.98, min_extrema=False, verbose=True):
        ...
        g, s, l = self.__grimshaw()
        self.extreme_quantile = self.__quantile(g, s)
    ...
    def __rootsFinder(fun, jac, bounds, npoints, method):
        """
    ...
    def __log_likelihood(Y, gamma, sigma):
        """
    ...
    def __grimshaw(self, epsilon=1e-8, n_points=10):
        """
        ...
        left_zeros = SPOT.__rootsFinder(lambda t: w(self.peaks, t),
                                        lambda t: jac_w(self.peaks, t),
                                        (a + epsilon, -epsilon),
                                        n_points, 'regular')

        right_zeros = SPOT.__rootsFinder(lambda t: w(self.peaks, t),
                                         lambda t: jac_w(self.peaks, t),
                                         (b, c),
                                         n_points, 'regular')
        ...
        gamma_best = 0
        sigma_best = Ymean
        ll_best = SPOT.__log_likelihood(self.peaks, gamma_best, sigma_best)

        # we look for better candidates
        for z in zeros:
            gamma = u(1 + z * self.peaks) - 1
            sigma = gamma / z
            ll = SPOT.__log_likelihood(self.peaks, gamma, sigma)
            if ll > ll_best:
                gamma_best = gamma
                sigma_best = sigma
                ll_best = ll
    ...
    def __quantile(self, gamma, sigma):
        """
