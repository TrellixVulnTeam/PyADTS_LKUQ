...
from pyadts.data.preprocessing import series_rearrange, series_normalize
...
def test_series_normalize():
    ...
    for method in ['minmax', 'negpos1', 'zscore']:
        data_df_cp = copy.deepcopy(data_df)
        series_normalize(data_df_cp, method=method)
        if method == 'minmax':
            assert np.isclose(np.nanmin(data_df_cp.values, axis=0),
                              np.zeros(2)).all(), f'{np.nanmin(data_df_cp.values, axis=0)} - {np.zeros(2)}'
            assert np.isclose(np.nanmax(data_df_cp.values, axis=0),
                              np.ones(2)).all(), f'{np.nanmax(data_df_cp.values, axis=0)} - {np.ones(2)}'
        elif method == 'negpos1':
            assert np.isclose(np.nanmin(data_df_cp.values, axis=0),
                              np.full(fill_value=-1, shape=(2,))).all(), f'{np.nanmin(data_df_cp.values, axis=0)}'
            assert np.isclose(np.nanmax(data_df_cp.values, axis=0),
                              np.ones(2)).all(), f'{np.nanmax(data_df_cp.values, axis=0)}'
        else:
            assert np.isclose(np.nanmean(data_df_cp.values, axis=0),
                              np.zeros(2)).all(), f'{np.nanmean(data_df_cp.values, axis=0)}'
